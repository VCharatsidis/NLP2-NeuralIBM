...
class NeuralIBM1Model:
  ...
  def _create_weights(self):
    ...
    with tf.variable_scope("MLP") as scope:
      self.W1 = tf.get_variable(name="W1", initializer=glorot_uniform(),
        shape=[self.emb_dim, self.mlp_dim])

      self.b1 = tf.get_variable(
        name="b1", initializer=tf.zeros_initializer(),
        shape=[self.mlp_dim])

      self.W2 = tf.get_variable(
        name="W2", initializer=glorot_uniform(),
        shape=[self.mlp_dim, self.y_vocabulary_size])

      self.b2 = tf.get_variable(
        name="b2", initializer=tf.zeros_initializer(),
  ...
  def _build_model(self):
    ...
    h = tf.matmul(mlp_input, self.W1) + self.b1
    h = tf.tanh(h)
